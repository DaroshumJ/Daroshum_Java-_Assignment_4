...
public class Ink {
  ...
  public void closeAccount(ArrayList<Stock> stocks) {
    double totalAmount = 0.0;
    for (Stock stock : stocks) {
        totalAmount += stock.getPrice() * stock.getQty();
        stock.setQty(0); // Set quantity to 0 when sold 
    }
    stocks.clear(); // Clear all stocks from the portfolio
    System.out.printf("All stocks have been sold. Total amount earned: $%.2f\n", totalAmount);
}
} // class